...
class SmoothCircleCommand(BaseSmoothMotionCommand):
    ...
    def generate_main_trajectory(self, effective_start_pose):
        ...
        if abs(distance_to_center - self.radius) > self.radius * 0.3:
            logger.warning(f"    WARNING: Robot is {distance_to_center:.1f}mm from center, but radius is {self.radius:.1f}mm")
            logger.info(f"    Circle geometry will be auto-corrected")
...
class SmoothSplineCommand(BaseSmoothMotionCommand):
    ...
    def generate_main_trajectory(self, effective_start_pose):
        ...
        if first_wp_error > 5.0:
            ...
            logger.error(f"    Added start position as first waypoint (distance: {first_wp_error:.1f}mm)")
        else:
...
class SmoothBlendCommand(BaseSmoothMotionCommand):
    ...
    def generate_main_trajectory(self, effective_start_pose):
        ...
        if len(trajectories) > 1:
            ...
            logger.info(f"    Blended {len(trajectories)} segments into {len(blended)} points")
            return blended
        elif trajectories:
...
def parse_smooth_motion_commands(parts):
    ...
    try:
        if command_type == 'SMOOTH_CIRCLE':
            ...
            logger.info(f"  -> Parsed circle: r={radius}mm, plane={plane}, frame={frame}, {timing_type}={timing_value}, duration={duration:.2f}s")
            ...
            logger.info(f"  -> Parsed arc (center): frame={frame}, {timing_type}={timing_value}, duration={duration:.2f}s")
            ...
            logger.info(f"  -> Parsed arc (param): r={radius}mm, θ={arc_angle}°, frame={frame}, duration={duration:.2f}s")
            ...
            logger.info(f"  -> Parsed spline: {num_waypoints} points, frame={frame}, duration={duration:.2f}s")
            ...
            logger.info(f"  -> Parsed helix: h={height}mm, pitch={pitch}mm, frame={frame}, duration={duration:.2f}s")
